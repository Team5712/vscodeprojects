...
public class Limelight extends SubsystemBase {
  ...
  NetworkTableEntry tthor = table.getEntry("thor");
  ...
  double thor = tthor.getDouble(0.0);
  ...
  public boolean alignGood() {
    if (tx.getDouble(0.0) > -2 && tx.getDouble(0.0) < 2) {
      return true;
    } else {
      return false;
    }
  ...
  public double turnToTarget() {
    ...
    if (targetingOn) {
      ...
      // double rcw = kP * error + kD * derivative;
      previous_error = error;
      if(Math.abs(error) < 0.25) {
        integral = 0;
      }

      return Math.max(Math.min(rcw, .4), -.4);
    }
    return 0;
  }

  public double[] calcHoodAndRPM() {
    System.out.println("LIMELIGHT" + tthor.getDouble(0.0));
    //ty rpm hood
    //{220, 11500, -3.25}
    //1000-thor
    double arr[][] = {
    ...
    double yoffset = tthor.getDouble(0.0);
    int ans = 0;
    while (start <= end) {
      int mid = (start + end) / 2;
